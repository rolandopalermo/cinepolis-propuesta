...
\item Redis puede mostrar un mapa de asientos más rápido, pero el último asiento lo confirma PostgreSQL.
\end{itemize}
...
\begin{tikzpicture}[remember picture, overlay]
  \node[inner sep=0pt, anchor=center] at (current page.center) {%
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{Checkout API Admission Flow-2026-09-07-055408.png}%
  };
\end{tikzpicture}
\clearpage
...
\begin{itemize}
\item La pasarela autoriza el pago; LiveTix no almacena tarjetas.
\item La captura ocurre solo cuando el boleto ya se generó.
\item Si la emisión falla de forma permanente, se hace \textit{void} de la autorización y la orden queda cancelada.
\item Un reconciliador compara periódicamente pagos, órdenes y boletos.
\end{itemize}
...
\begin{tikzpicture}[remember picture, overlay]
  \node[inner sep=0pt, anchor=center] at (current page.center) {%
    \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{checkout-api-admission-flow-060456.png}%
  };
\end{tikzpicture}
\clearpage
